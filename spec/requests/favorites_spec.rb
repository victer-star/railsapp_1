...
RSpec.describe "お気に入り登録機能", type: :request do
  ...
  context "お気に入り登録処理" do
    context "ログインしている場合" do
      ...
      it "筋トレメニューのお気に入り登録ができること" do
        expect {
          post "/favorites/#{training.id}/create"
        }.to change(user.favorites, :count).by(1)
      end

      it "筋トレメニューのAjaxによるお気に入り登録ができること" do
        expect {
          post "/favorites/#{training.id}/create", xhr: true
        }.to change(user.favorites, :count).by(1)
      end

      it "筋トレメニューのお気に入り解除ができること" do
        user.favorite(training)
        expect {
          delete "/favorites/#{training.id}/destroy"
        }.to change(user.favorites, :count).by(-1)
      end

      it "筋トレメニューのAjaxによるお気に入り解除ができること" do
        user.favorite(training)
        expect {
          delete "/favorites/#{training.id}/destroy", xhr: true
        }.to change(user.favorites, :count).by(-1)
      end
    end
